...
class Residual(nn.Module):
    def __init__(self, input_channels, num_channels, use_1conv=False,strides=1):
        ...
        self.ReLU=nn.ReLU()
        self.conv1 = nn.Conv2d(in_channels=input_channels, out_channels=num_channels, kernel_size=3, stride=strides, padding=1, bias=False)
        ...
        if use_1conv:
            self.conv3=nn.Conv2d(in_channels=input_channels,out_channels=num_channels,kernel_size=1,stride=strides,bias=False)
        else:
            self.conv3=None
    def forward(self, x):
        y=self.ReLU(self.bn1(self.conv1(x)))
        y=self.bn2(self.conv2(y))    # bn层是为了减少梯度消失，防止梯度爆炸，
        #bn是指归一化，即对输入数据进行线性变换，使其分布于0~1之间，便于后续的计算。
